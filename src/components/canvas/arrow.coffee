...
class Arrow extends Stimulus
  ...
  render: (context, layer) ->
    ...
    group = new Kinetic.Group({x: 0, y: 0, rotationDeg: @spec.angle, offset: [0, @spec.thickness / 2.0]})
    group.add(rect)
    group.add(triangle)

    coords = @computeCoordinates(context, @spec.position, group.getWidth(), group.getHeight())
    group.setPosition({x: coords[0], y: coords[1]})
    console.log("group width:", group.getWidth())
    console.log("group height:", group.getHeight())
    layer.add(group)
